...
PYBIND11_MODULE(pbctools_cpp, m) {
  ...
  m.def("molecule_recognition", &pbctools::molecule_recognition,
          py::arg("coords"), py::arg("atoms"), py::arg("pbc"), py::arg("return_indices") = false,
          "Identify molecular species in single frame (coords:(N,3) float32). "
          "Returns dict formula->count, or if return_indices=True, a tuple "
          "(formula->count dict, list of {'formula', 'indices'} dicts giving the "
          "0-based atom indices of each detected molecule).");
